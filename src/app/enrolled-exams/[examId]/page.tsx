...
function ExamDashboardPage({ params }: { params: { examId: string } }) {
  ...
  useEffect(() => {
    if (!loading && !user) {
      router.push(`/login?redirect=/enrolled-exams/${examId}`);
    }
        ...
        <main className="p-4 md:p-6 lg:p-8">
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Access all your study materials, practice tests, and videos for the {exam.examName} here.
                    </CardDescription>
                </CardHeader>
            </Card>
...
export default function ExamDashboardPageWrapper({ params }: { params: { examId: string } }) {
  return (
    <AuthProvider>
      <ExamDashboardPage params={params} />
    </AuthProvider>
  );
}
